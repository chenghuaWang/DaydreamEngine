...
Texture2D::Texture2D(uint32_t w, uint32_t h) : _obj_texture(w, h, 4, COLOR_SPACE::RGBA) {
  m_name = "No data";
  m_path = "default";
  ...
  if (!hdr) {
    ...
    switch (m_c) {
      ...
      default:
        ...
        m_format = 0;
        LOG_ERROR("When loading texture. Found channel is " + std::to_string(m_c)
                  + ". can't handle.");
        break;
    }
...
Texture2D::~Texture2D() { glDeleteTextures(1, &m_idx); }

void Texture2D::Bind(uint32_t b) const { glBindTextureUnit(b, m_idx); }

void Texture2D::UnBind(uint32_t b) const { glBindTextureUnit(b, 0); }
...
bool Texture2D::isEmpty() { return m_hasImage; }

REF(Texture2D) Texture2D::create(uint32_t w, uint32_t h) { return CREATE_REF(Texture2D)(w, h); }
