...
void _obj_light::resetColor(glm::vec3& v) { m_color = v; }
...
LightDirect::LightDirect() { m_type = LightType::Direct; }

LightDirect::LightDirect(const glm::vec3& direction) : m_direction(direction) {
  m_type = LightType::Direct;
}

void LightDirect::resetDirection(const glm::vec3& v) { m_direction = v; }
...
LightPoint::LightPoint() { m_type = LightType::Point; }
...
void LightPoint::resetLinear(float v) { m_linear = v; }

void LightPoint::resetQuadratic(float v) { m_quadratic = v; }

// -------------------------- Light Spot type ------------------------------

LightSpot::LightSpot() { m_type = LightType::Spot; }
...
void LightSpot::resetCutOff_radians(float v) { m_cutoff = glm::cos(glm::radians(v)); }

void LightSpot::resetOutCutOff(float v) { m_outCutoff = v; }

void LightSpot::resetOutCutOff_radians(float v) { m_outCutoff = glm::cos(glm::radians(v)); }

void LightSpot::resetDirection(const glm::vec3& v) { m_direction = v; }
