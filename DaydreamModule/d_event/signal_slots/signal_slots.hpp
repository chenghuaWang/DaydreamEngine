...
class Signal0 {
 ...
 public:
  void Connect(const _Delegate& delegate) { delegateList.insert(delegate); }
  ...
  void Disconnect(const _Delegate& delegate) { delegateList.erase(delegate); }
  ...
  void Emit() const {
    for (auto i = delegateList.begin(); i != delegateList.end();) { (*(i++))(); }
  }

  void operator()() const { Emit(); }

  [[nodiscard]] bool Empty() const { return delegateList.empty(); }
};
...
class Signal1 {
 ...
 public:
  ...
  void Emit(Param1 p1) const {
    for (DelegateIterator i = delegateList.begin(); i != delegateList.end();) { (*(i++))(p1); }
  }
...
class Signal2 {
 ...
 public:
  ...
  void Emit(Param1 p1, Param2 p2) const {
    for (DelegateIterator i = delegateList.begin(); i != delegateList.end();) { (*(i++))(p1, p2); }
  }
