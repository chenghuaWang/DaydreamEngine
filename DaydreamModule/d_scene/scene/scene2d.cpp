...
namespace scene {
	...
	scene2d::scene2d(const renderer::camera2d& camera) :m_MainCamera(camera), m_FramBuffer(camera.m_w, camera.m_h) {
	}
	...
	void scene2d::setLineMode(bool enable) {
		m_LineMode = enable;
	}

	bool scene2d::isLineMode() {
		return m_LineMode;
	}

	void scene2d::setCurShader(REF(renderer::Shader)& shader) {
		m_MainShader = shader;
	}
