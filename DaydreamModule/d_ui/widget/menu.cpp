...
void sub_menu::add_action(const action& rhs) { m_actions.push_back(rhs); }
...
void menu::add_sub_menu(sub_menu& rhs) { m_sub_menus.push_back(rhs); }
...
void menu::remove_sub_menu(sub_menu& rhs) {
  for (auto it = m_sub_menus.begin(); it != m_sub_menus.end();) {
    if (it->name() == rhs.name()) { m_sub_menus.erase(it); }
    ++it;
  }
}
...
void menu::impl_imgui_render() {
  if (ImGui::BeginMenuBar()) {
    for (auto& item : m_sub_menus) { item.impl_imgui_render(); }
    ImGui::EndMenuBar();
  }
}
