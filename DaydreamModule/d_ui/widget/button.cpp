...
namespace ui {
	...
	image_button::~image_button() {
		glDeleteTextures(1, &m_image_idx);
	}

	void image_button::impl_imgui_render() {
		if (ImGui::ImageButton((void*)m_image_idx, m_image_size)) {
			click.Emit();
		}
	}
	...
	void label::impl_imgui_render() {
		ImGui::LabelText(m_value.c_str(), m_name.c_str());
	}

	void label::set_value(const std::string& v) {
		m_value = v;
	}
