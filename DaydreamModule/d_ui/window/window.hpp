 ...
 *			You need to use window-flags to init this window class first when you want
 *to start a thread with imgui window.
 *
 * For Auto DPI, check the https://github.com/ocornut/imgui/issues/2442.
 */
class D_API_EXPORT imgui_window : public window_base {
 public:
  imgui_window(size_t w = 1920, size_t h = 1080,
               uint16_t flags = imgui_window_flags::AUTO_DPI | imgui_window_flags::DOCKING
                                | imgui_window_flags::MULTI_WINDOW,
