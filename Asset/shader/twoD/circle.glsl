...
uniform vec4 iMouse;       // mouse pixel coords. xy: current (if MLB down), zw: click
                           // gl_FragCoord is the gl_FragCoord in shadertoy
...
void main() {
  ...
  if (iMouse.z < 1.0) { mm = vec2(sin(iTime / 6.) / 1., cos(iTime / 8.) / 2.) / 2.; }
  ...
  // this calls the function which adds three circle types every time through the loop based on
  // parameters I got by trying things out. rnd i*2000. and rnd i*20 are just to help randomize
  // things more
  for (float i = 0.; i < 10.; i++) {
    color += circle(uv, pow(rnd(i * 2000.) * 1.8, 2.) + 1.41, 0.0, circColor + i, circColor2 + i,
                    rnd(i * 20.) * 3. + 0.2 - .5, mm);
  }
  ...
  float bright = 0.1;  //+0.1/abs(sin(iTime/3.))/3.;//add brightness based on how the sun moves so
                       // that it is brightest
  ...
  color += max(0.1 / pow(length(uv - mm) * 10., 1. / 20.), .0)
           + abs(sin(a * 3. + cos(a * 9.))) / 8. * (abs(sin(a * 9.))) / 1.;
  // add another sun in the middle (to make it brighter)  with the20color I want, and bright as the
  // numerator.
